...
configureSystem() {
    ...
    if [[ $(uname) == "Darwin" ]]; then
        ...
        # Reduce Motion - disable animations when opening and closing windows
...
configureSoftware() {
    ...
    command -v bat &>/dev/null && {
        if ! cat $(bat --config-file) | grep -q "\-\-theme\="; then
            ...
            local themes_dir="$(dirname $config_dir)/themes"
            git clone -v git@github.com:catppuccin/bat.git $tmp
            mkdir -p "$themes_dir"
            cp -f "${tmp}"/*.tmTheme "$themes_dir/" &&
                bat cache --build &&
...
installSoftware() {
    ...
    # Upgrades casks currently installed • https://stackoverflow.com/a/31994862/971329
